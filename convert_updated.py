...
def listdir_ext(directory, extension):
    return [f for f in os.listdir(directory) if f.endswith(extension)]

# Path to the directory where checkpoint files are stored
voc_dir = args.voc_dir
checkpoint_dir = os.path.join(voc_dir,'exp','train_nodev_all_{}'.format(vocoder))

# Get a list of checkpoint files with the .pkl extension
checkpointlist = listdir_ext(checkpoint_dir, '.pkl')

# Check if checkpoint list is not empty
if checkpointlist:
    # Sort the list of checkpoints, assuming the file naming is such that
    # the latest checkpoint has the highest sorting order
    checkpointlist.sort()
    # Use the last checkpoint in the sorted list
    nv_checkpoint = os.path.join(checkpoint_dir, checkpointlist[-1])
    print('vocoder: {}'.format(os.path.abspath(nv_checkpoint)))
else:
    # If the list is empty, print an error message and exit
    print(f'Error: No checkpoint files found in directory {checkpoint_dir}')
    sys.exit(1)
...
# Load the model without weight normalization and set to evaluation mode
