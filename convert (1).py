...
def listdir_ext(dirpath, ext):
    ...
    if os.path.isfile(dirpath):
        # If dirpath is a file, return the file directly
        return [dirpath]
    else:
        # If dirpath is a directory, list all files with the specified extension
...
def main():
    ...
    parser.add_argument('--dataconf', type=str, default='/workspaces/StarGAN-VC/dump/mydataset/data_config.json')
    ...
    data_config_path = './dump/mydataset/data_config.json'
    model_config_path = '/workspaces/StarGAN-VC/model/mydataset/conv_wgan_exp1/model_config.json'
    ...
    stat_filepath = '/workspaces/StarGAN-VC/dump/mydataset/stat.pkl'
    ...
    # Set up nv
    voc_dir = args.voc_dir  # Assuming this is either a directory or a .pkl file path
    voc_yaml_path = os.path.join(voc_dir, 'conf', '{}.yaml'.format(vocoder)) if os.path.isdir(voc_dir) else os.path.join(os.path.dirname(voc_dir), 'conf', '{}.yaml'.format(vocoder))
    checkpointlist = listdir_ext(
        os.path.join(voc_dir, 'exp', 'train_nodev_all_{}'.format(vocoder)) if os.path.isdir(voc_dir) else voc_dir,
        '.pkl')
    nv_checkpoint = checkpointlist[-1]  # Di
